...
<# Describe "Silent install" {
    It "Setup installs files and shortcuts" {
        ...
        $path | Should Exist
        Join-Path $path "ETPlayer.DesktopApp.exe" | Should Exist
        (Get-ChildItem $path).Count | Should BeGreaterThan 1
    }

    It "ETPlayer is started after installation" {
        Get-Process -Name "ETPlayer.DesktopApp" -EA 0 | Should Not BeNullOrEmpty
        WaitFor { Get-UIWindow -ProcessName "ETPlayer.DesktopApp" } 10000
    }

    It "Shortucts start ETPlayer" {
        if ($proc = Get-Process -Name "ETPlayer.DesktopApp.exe" -EA 0)
        {
        ...
        $path = Join-Path $dektop "ETPlayer.lnk"
        $path | Should Exist
        Start-Process $path

        Get-Process -Name "ETPlayer.DesktopApp" -EA 0 | Should Not BeNullOrEmpty
        Stop-Process -Name "ETPlayer.DesktopApp"
    }
}

Describe "Uninstall" {
    It "Uninstall removes all files" {
        $window = { Get-UIDesktop | Get-UIControl -ControlType "Dialog" -Name "ETPlayer*Uninstall" }
        $testData = Get-TestProperties

        $dektop = [System.Environment]::ExpandEnvironmentVariables("%systemdrive%\users\%username%\Desktop")
        $startMenu = [System.Environment]::ExpandEnvironmentVariables("c:\Users\crow\AppData\Roaming\Microsoft\Windows\Start Menu\Programs\ETPlayer\")        
        ...
        Join-Path $dektop "ETPlayer.lnk" | Should Not Exist
