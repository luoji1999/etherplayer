...
param(
    ...
    [string]$TempDir = (Join-Path $env:TEMP "ETPlayerBuild"),
...
if (!$SkipBuild)
{
    ...
    Invoke-Nuget "restore ..\source\ETPlayer.sln"
...
if ($Package)
{
    ...
    $packageName = Join-Path $InstallerDir "ETPlayer.zip"
    New-ZipFromDirectory $OutputDir $packageName
}

(Get-ChildItem (Join-Path $OutputDir "ETPlayer.dll")).VersionInfo.FileVersion | Write-Host -ForegroundColor Green
return $true
