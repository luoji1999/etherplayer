...
namespace Playnite
{
    ...
    public class PlayniteUriHandler : IUriHandlerAPI
    {
        ...
        public void RegisterSource(string source, Action<PlayniteUriEventArgs> handler)
        {
            ...
            if (source.Equals("etplayer", StringComparison.OrdinalIgnoreCase))
            {
                throw new Exception($"\"etplayer\" source is reserved source.");
            }
        ...
        public static (string source, string[] arguments) ParseUri(string uri)
        {
            ...
            if (split.Length < 2)
            {
                throw new Exception("etplayer:// uri missing parameters.");
            }
        ...
        {
            logger.Info($"Processing ETPlayer URI {uri}");
            try
            {
