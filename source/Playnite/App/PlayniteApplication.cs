...
namespace Playnite
{
    public abstract class PlayniteApplication : ObservableObject, IPlayniteApplication
    {
        ...
        private const string instanceMuxet = "ETPlayerInstaceMutex";
            ...
            CmdLineOptions cmdLine)
        {
            ...
            {
                if (MessageBox.Show(
                    "ETPlayer closed unexpectedly while starting. This is usually caused by 3rd party theme or extension. Do you want to start in safe mode with all 3rd party add-ons disabled?",
                    "Startup Error",
        ...
        private void CurrentDomain_UnhandledException(object sender, UnhandledExceptionEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    Dialogs.ShowErrorMessage("System issue or corrupted ETPlayer install detected.");
                    Process.GetCurrentProcess().Kill();
                    return;
                }
        ...
        public bool CheckOtherInstances()
        {
            ...
            if (Mutex.TryOpenExisting(instanceMuxet, out var mutex))
            {
                ...
                {
                    MessageBox.Show(
                        "ETPlayer failed to start. Please close all other instances and try again.",
                        "Startup Error");
        ...
        public bool ConfigureApplication()
        {
            ...
            {
                logger.Error(exc, "Failed to register ETPlayer to start on boot.");
            }
        ...
        {
            logger.Info("Shutting down ETPlayer");
            if (saveSettings)
            {
                AppSettings?.SaveSettings();
            }
        ...
        {
            logger.Info("Shutting down ETPlayer and starting an app.");
            if (saveSettings)
            {
                AppSettings?.SaveSettings();
            }
        ...
        {
            logger.Info("Shutting down ETPlayer and executing an action.");
            if (saveSettings)
            {
                AppSettings?.SaveSettings();
            }
        ...
        public virtual void ReleaseResources(bool releaseCefSharp = true)
        {
            ...
            logger.Debug("Releasing ETPlayer resources...");
            ...
            Dialogs.ActivateGlobalProgress(_ =>
            {
                ...
                {
                    logger.Error(exc, "Failed to dispose ETPlayer objects.");
                }
        ...
        private void WaitForOtherInstacesToExit(bool throwOnTimetout)
        {
            ...
            {
                logger.Info("Multiple ETPlayer instances detected, waiting for them to close.");
                for (int i = 0; i < 10; i++)
                {
                    Thread.Sleep(500);
                    if (Process.GetProcesses().Where(a => IsProcessPlayniteProcess(a)).Count() == 1)
                    ...
                    else if (i == 9)
                    {
                        if (throwOnTimetout)
                        {
                            throw new Exception("Another ETPlayer instance didn't shutdown in time.");
                        }
                        else
                        {
                            logger.Warn("Another ETPlayer instance didn't shutdown in time.");
                        }
                    }
                }
            }
        ...
        {
            return process.ProcessName.StartsWith("ETPlayer.DesktopApp") || process.ProcessName.StartsWith("ETPlayer.FullscreenApp");
        }
