    ...
    public static class Diagnostic
    {
        ...
        private static List<string> GetPlayniteFilesList()
        {
            ...
            foreach (var file in Directory.GetFiles(progPath, "*.*", SearchOption.TopDirectoryOnly))
            {
                ...
                {
                    logger.Error(e, $"Failed to get information about ETPlayer file {file}");
                }
            }
            ...
            using (FileStream zipToOpen = new FileStream(path, FileMode.Open))
            {
                using (ZipArchive archive = new ZipArchive(zipToOpen, ZipArchiveMode.Update))
                {
                    ...
                    var playniteInfo = new Dictionary<string, object>
                    {
                        ...
                        { "ETPlayer.DesktopApp.exe_MD5", FileSystem.GetMD5(PlaynitePaths.DesktopExecutablePath) },
                        { "ETPlayer.FullscreenApp.exe_MD5", FileSystem.GetMD5(PlaynitePaths.FullscreenExecutablePath) },
                        { "ETPlayer.dll_MD5", FileSystem.GetMD5(PlaynitePaths.PlayniteAssemblyPath) },
                        { "ETPlayer.SDK.dll_MD5", FileSystem.GetMD5(PlaynitePaths.PlayniteSDKAssemblyPath) }
                    };
