...
namespace PlayniteInstaller
{
    ...
    public class MainViewModel : ObservableObject
    {
        ...
        {
            windowHost = window;
            DestionationFolder = Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.LocalApplicationData), "ETPlayer");
        ...
        public void Browse()
        {
            ...
            {
                DestionationFolder = Path.Combine(dialog.SelectedPath, "ETPlayer");
            }
        }
        ...
        public async Task Install()
        {
            if (DestionationFolder.StartsWith(@"c:\Program Files", StringComparison.OrdinalIgnoreCase))
            {
                System.Windows.MessageBox.Show(
                    "Can't install ETPlayer to selected directory, use a different path.",
                ...
                await Task.Run(() =>
                {
                    using (var process = Process.Start(App.InstallerDownloadPath, args))
                    {
                        ...
                        else
                        {
                            ...
                            System.Windows.MessageBox.Show(
                                $"Failed to install ETPlayer. Error code {process.ExitCode}",
                                "Installation error",
                                MessageBoxButton.OK, MessageBoxImage.Error);
            ...
            {
                logger.Error(e, "Failed to download and install ETPlayer.");
                System.Windows.MessageBox.Show(
                    $"Failed to install ETPlayer:\n{e.Message}",
                    "Installation error",
                    MessageBoxButton.OK, MessageBoxImage.Error);
                Status = InstallStatus.Idle;
            }
