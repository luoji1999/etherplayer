...
namespace Playnite.FullscreenApp
{
    public class ProgramEntry
    {
        ...
        public static void Main(string[] args)
        {
            ...
            {
                MessageBox.Show(
                     "Windows 7 and Windows 8 are no longer supported. Please update your operating system or downgrade to older ETPlayer version.",
                     "Startup Error",
                     MessageBoxButton.OK,
                     MessageBoxImage.Error);
                return;
            ...
            {
                MessageBox.Show(
                    "ETPlayer is not allowed to run from temporary extracted archive.\rInstall/Extract application properly before starting it.",
                    "Startup Error",
                    MessageBoxButton.OK,
                    MessageBoxImage.Error);
                return;
            ...
            {
                MessageBox.Show(
                    "ETPlayer is unable to run from current directory due to illegal character '#' in the path. Please use different directory.",
                    "Startup Error",
                    MessageBoxButton.OK,
                    MessageBoxImage.Error);
                return;
